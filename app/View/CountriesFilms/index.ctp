<div class="countriesFilms index">
	<h2><?php echo __('Countries Films');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('country_id');?></th>
			<th><?php echo $this->Paginator->sort('film_id');?></th>
			<th class="actions"><?php echo __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($countriesFilms as $countriesFilm): ?>
	<tr>
		<td><?php echo h($countriesFilm['CountriesFilm']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($countriesFilm['Country']['name'], array('controller' => 'countries', 'action' => 'view', $countriesFilm['Country']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($countriesFilm['Film']['id'], array('controller' => 'films', 'action' => 'view', $countriesFilm['Film']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $countriesFilm['CountriesFilm']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $countriesFilm['CountriesFilm']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $countriesFilm['CountriesFilm']['id']), null, __('Are you sure you want to delete # %s?', $countriesFilm['CountriesFilm']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Countries Film'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Countries'), array('controller' => 'countries', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Country'), array('controller' => 'countries', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Films'), array('controller' => 'films', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Film'), array('controller' => 'films', 'action' => 'add')); ?> </li>
	</ul>
</div>
